...
class LobbySidebar(private val player: Player): Sidebar("lobby_${player.name}", player, CommandSenderUtil.getComponent(player, "lobby", "scoreboard_title")) {
    ...
    init {
        ...
        if (permissionUser != null) {
            ...
            updateScore(7, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_rank_title")))
            updateScore(6, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_rank_value",
                "{rank}" to legacyAmpersand.serialize(Component.text(permissionGroup.name, permissionGroup.color)))))
            updateScore(5, "&a")
            updateScore(4, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_currency_title")))
            updateScore(3, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_currency_value", "{currency}" to 0)))
            updateScore(2, "&c")
            updateScore(1, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_server_title")))
            updateScore(0, legacyAmpersand.serialize(CommandSenderUtil.getComponent(player, "lobby", "scoreboard_server_value", "{server}" to "Lobby-1")))
        }
    }
}